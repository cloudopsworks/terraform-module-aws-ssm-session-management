...
#   create_grant_iam_role_arns: []                  # (Optional) IAM role ARNs granted kms:CreateGrant on the module KMS key. Grants no S3 access. Merged with whatever create_grant_iam_role_names resolves to. Use for cross-account roles, which cannot be looked up by name. Default: []
#   create_grant_iam_role_names: []                 # (Optional) IAM role names in the current account granted kms:CreateGrant on the module KMS key, for features that hand the key to another AWS service on the caller's behalf (Just-in-Time node access, RDP recording). Key policy only -- these roles get no bucket access. Same "*"/"?" wildcard support and plan-time resolution as allowed_iam_role_names, and each entry must contain at least one literal character. Default: []
...
variable "settings" {
  ...
  # Same guard as above, and it matters more here: these roles can delete audit objects.
  validation {
    condition = alltrue([
      for name in try(var.settings.admin_iam_role_names, []) : can(regex("[^*?]", name))
    ])
    error_message = "Each settings.admin_iam_role_names entry must contain at least one literal character. A pattern of only wildcards would grant every IAM role in the account full object access to the audit bucket."
  }

  # Same guard again: kms:CreateGrant lets the holder delegate key use to other principals.
  validation {
    condition = alltrue([
      for name in try(var.settings.create_grant_iam_role_names, []) : can(regex("[^*?]", name))
    ])
    error_message = "Each settings.create_grant_iam_role_names entry must contain at least one literal character. A pattern of only wildcards would let every IAM role in the account create grants on the KMS key."
  }
  ...
  validation {
    condition = !try(var.settings.fleet_manager.remote_desktop.recording.enabled, false) || try(var.settings.fleet_manager.remote_desktop.recording.just_in_time_node_access.enabled, false)
    error_message = join("", [
  ...
  validation {
    condition     = !try(var.settings.fleet_manager.remote_desktop.recording.enabled, false) || !try(var.settings.fleet_manager.remote_desktop.recording.just_in_time_node_access.organization_level, false) || try(var.settings.fleet_manager.remote_desktop.recording.just_in_time_node_access.target_organizational_units, "") != ""
    error_message = "settings.fleet_manager.remote_desktop.recording.just_in_time_node_access.target_organizational_units is required when RDP recording is enabled and just_in_time_node_access.organization_level is set to true. Supply a comma separated list of organizational unit IDs, or the organization root ID to cover the whole organization -- or leave organization_level at its default of false to set just-in-time node access up for this account alone."
  }
