...
locals {
  ssm_logs_bucket = try(var.settings.random_bucket_suffix, true) && !try(var.settings.organization.delegated, false) ? "ssm-session-auditlogs-${local.system_name}-${random_string.random[0].result}" : "ssm-session-auditlogs-${local.system_name}"
